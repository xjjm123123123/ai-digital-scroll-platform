...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    let backgroundAspect = 1.0;
    let isBackgroundLoaded = false;
    
    const updateUniformsWithAspect = (aspect: number) => {
      backgroundAspect = aspect;
      if (webglRef.current?.output?.output?.material instanceof THREE.RawShaderMaterial) {
        webglRef.current.output.output.material.uniforms.uTextureAspect.value = aspect;
        webglRef.current.output.output.material.uniforms.uHasBackground.value = true;
        
        console.log('LiquidEther Image Loaded:', {
          textureAspect: aspect,
          viewAspect: Common.aspect,
          ratio: Common.aspect / aspect,
          mode: Common.aspect / aspect < 1.0 ? 'scroll' : 'wide-screen'
        });
        ...
        requestAnimationFrame(() => {
          if (webglRef.current?.output) {
            webglRef.current.output.update();
          }
        });
      }
    };
    ...
    if (backgroundImage) {
      ...
      backgroundTex = loader.load(backgroundImage, (tex) => {
        if (tex.image && tex.image.width > 0 && tex.image.height > 0) {
          ...
          // 如果 WebGL 管理器已经初始化，立即更新 uniforms
          if (webglRef.current?.output?.output?.material instanceof THREE.RawShaderMaterial) {
            updateUniformsWithAspect(aspect);
          } else {
            // 如果 WebGL 管理器还没有初始化，等待初始化完成
            const checkAndInit = () => {
              if (webglRef.current?.output?.output?.material instanceof THREE.RawShaderMaterial) {
                updateUniformsWithAspect(aspect);
              } else {
      ...
      backgroundTex.wrapS = THREE.ClampToEdgeWrapping;
      backgroundTex.wrapT = THREE.ClampToEdgeWrapping;
    }
...
`;
    ...
    class Output {
        ...
        this.output = new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            uniforms: {
              ...
              uHasBackground: { value: isBackgroundLoaded },
              uTime: { value: 0 },
      ...
      resize() {
        ...
        if (this.output.material instanceof THREE.RawShaderMaterial) {
          this.output.material.uniforms.uViewAspect.value = Common.aspect;
          
          // 同步纹理比例
          const tex = this.output.material.uniforms.uBackground.value;
          if (tex && tex.image && tex.image.width > 0) {
            this.output.material.uniforms.uTextureAspect.value = tex.image.width / tex.image.height;
          }
        }
      }
      render() {
      ...
      update() {
        ...
        if (this.output.material instanceof THREE.RawShaderMaterial) {
          ...
          const tex = mat.uniforms.uBackground.value;
          
          if (tex && tex.image && tex.image.width > 0 && isBackgroundLoaded) {
            const aspect = tex.image.width / tex.image.height;
            const ratio = mat.uniforms.uViewAspect.value / aspect;
            
            if (ratio < 1.0) {
              this.scrollOffset += this.scrollSpeed;
              const maxOffset = 1.0 - ratio;
              if (this.scrollOffset > maxOffset) {
                this.scrollOffset = 0;
              }
            }
            
            if (Math.abs(mat.uniforms.uTextureAspect.value - aspect) > 0.001) {
              mat.uniforms.uTextureAspect.value = aspect;
            }
            
            const hasBg = mat.uniforms.uHasBackground.value;
            if (Math.abs((hasBg ? 1 : 0) - (isBackgroundLoaded ? 1 : 0)) > 0.01) {
              mat.uniforms.uHasBackground.value = isBackgroundLoaded;
            }
            
            if (Math.floor(Common.time * 60) % 60 === 0) {
              console.log('LiquidEther Frame:', {
                viewAspect: mat.uniforms.uViewAspect.value.toFixed(4),
                textureAspect: mat.uniforms.uTextureAspect.value.toFixed(4),
                ratio: ratio.toFixed(4),
                mode: ratio < 1.0 ? 'scroll' : 'wide-screen',
                scrollOffset: this.scrollOffset.toFixed(4),
                isBackgroundLoaded: isBackgroundLoaded,
                uHasBackground: mat.uniforms.uHasBackground.value
              });
            }
          } else {
            const hasBg = mat.uniforms.uHasBackground.value;
          ...
          mat.uniforms.uScrollOffset.value = this.scrollOffset;
        }
        this.render();
      }
    }
